...
- **Operational Mode (Coordination):** Điều phối hệ thống. Trong Mode này, Agent chỉ đơn thuần **gọi tool hệ thống** (ví dụ: `get_next_task`, `check_plans`) và **đọc directive từ Server**. Không thực hiện sửa đổi code trên local workspace, không tự ý suy luận lan man.
...
- **Auto-Pickup**: Khi gọi `complete_task`, nếu tham số `auto_pickup = true`, Server có thể ngay lập tức trả về task mới cho bạn thực hiện mà không cần bạn phải tự gọi `get_next_task` nữa.
...
1. **Nhận Task (Operational Mode)**:
   - Gọi tool `get_next_task(worker_id)`.
   - Kết quả trả về gồm `task_id` và chi tiết của task nằm inline ngay trong trường `task_details` kèm `action: EXECUTE`.
2. **Đọc Yêu cầu Task (Execution Mode)**:
   - Đọc trực tiếp nội dung yêu cầu ngay từ `task_details` của command trả về. Không cần load file qua thao tác phụ nào khác trừ khi task yêu cầu.
3. **Thực thi**:
...
5. **Báo cáo Hoàn thành**:
   - Gọi tool `complete_task(task_id, status="done", summary, worker_id, auto_pickup)`. 
   - Nếu `auto_pickup=true`, output của hàm này sẽ chứa task tiếp theo luôn. Tiến hành lặp lại bước 2 với task mới nhận.
6. **Lặp (Loop)**:
   - Lặp lại cho đến khi Server báo `IDLE` (ngưng, đợi loop timeout), hoặc chuyển Role (`BECOME_PLANNER`).
...
- `get_next_task`: Xin task để chạy.
