...
## Done Criteria
- [ ] `_archive/phase1/` chứa toàn bộ Phase 1 source
- [ ] `src/` chỉ còn `utils/file-backend.ts` + `utils/logger.ts`
- [ ] `prompts/` rỗng hoặc có .gitkeep
- [ ] `templates/` rỗng hoặc có .gitkeep
- [ ] `tasks/done/` rỗng
- [ ] `exchange/` subdirs rỗng
- [ ] `dist/` rỗng
- [ ] `tasks/README.md` updated (chỉ P2 tasks)
- [ ] `npm run build` pass
