...
## Done Criteria
- [ ] `LLMAdapter` interface defined with `health()` and `chat()`
- [ ] Unified types: `ChatRequest`, `ChatResponse`, `ChatMessage`, `ToolCall`
- [ ] `OllamaAdapter` implements `LLMAdapter`
- [ ] `createAdapter('ollama')` returns OllamaAdapter instance
- [ ] `createAdapter('gemini')` throws "not implemented" (placeholder for P2-05b)
- [ ] Error handling for connection refused, timeout, bad JSON
- [ ] `npm run build` pass
