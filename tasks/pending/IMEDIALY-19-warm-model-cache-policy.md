...
- [ ] Warm cache state is separate from runtime lease state
- [ ] Lease release does not always unload model
- [ ] Cache can be evicted by TTL or resource pressure
- [ ] Successor route can prefer warm model when safe
- [ ] Terminal table shows warm cache/loaded model state
- [ ] No fixed local VRAM number controls scheduling
