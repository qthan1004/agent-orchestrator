...
## Done Criteria
- [x] PlanWatcher quét registered workspace paths
- [x] Plan copy → runtime `plans/processing/`
- [x] Workspace plan gốc move sang `.agent/plans/processing/`
- [x] `complete_task` ghi `result-*.json` về `workspace/.agent/results/`
- [x] Backward compat: không có workspace → cũ vẫn chạy
