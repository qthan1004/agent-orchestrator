...
## Done Criteria
- [x] `spawn(payload)` → child process alive, receives stdin data
- [x] `kill(pid)` → graceful (SIGTERM) then force (SIGKILL)
- [x] Timeout auto-kill works
- [x] `worker:exit` event fires with correct data
- [x] `worker:timeout` event fires on timeout
- [x] `getActive()` returns current workers
