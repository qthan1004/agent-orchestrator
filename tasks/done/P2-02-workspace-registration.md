...
## Done Criteria
- [x] `WorkspaceRegistry.register('/path/to/project')` → `{ id: 'a1b2c3d4', path: '...' }`
- [x] `~/.orchestrator/workspaces/a1b2c3d4/` tồn tại
- [x] `workspaces.json` ghi mapping
- [x] Gọi lại cùng path → trả cùng ID (idempotent)
- [x] `register_worker` response có `workspace_id`
